...
class TrainingHyperParams:
    ...
    lr_rot_mul: float = TRAINING_BUILD_ARG_DEFAULTS["lr_rot_mul"]; lr_rot_stage1_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_rot_stage1_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_rot_mul"])); lr_rot_stage2_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_rot_stage2_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_rot_mul"])); lr_rot_stage3_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_rot_stage3_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_rot_mul"])); lr_rot_stage4_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_rot_stage4_mul", TRAINING_BUILD_ARG_DEFAULTS.get("lr_rot_stage3_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_rot_mul"])))
    lr_color_mul: float = TRAINING_BUILD_ARG_DEFAULTS["lr_color_mul"]; lr_color_stage1_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_color_stage1_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_color_mul"])); lr_color_stage2_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_color_stage2_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_color_mul"])); lr_color_stage3_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_color_stage3_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_color_mul"])); lr_color_stage4_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_color_stage4_mul", TRAINING_BUILD_ARG_DEFAULTS.get("lr_color_stage3_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_color_mul"])))
    lr_opacity_mul: float = TRAINING_BUILD_ARG_DEFAULTS["lr_opacity_mul"]; lr_opacity_stage1_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_opacity_stage1_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_opacity_mul"])); lr_opacity_stage2_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_opacity_stage2_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_opacity_mul"])); lr_opacity_stage3_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_opacity_stage3_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_opacity_mul"])); lr_opacity_stage4_mul: float = float(TRAINING_BUILD_ARG_DEFAULTS.get("lr_opacity_stage4_mul", TRAINING_BUILD_ARG_DEFAULTS.get("lr_opacity_stage3_mul", TRAINING_BUILD_ARG_DEFAULTS["lr_opacity_mul"])))
    ...
    use_sh_stage1: bool = TRAINING_BUILD_ARG_DEFAULTS["use_sh_stage1"]; use_sh_stage2: bool = TRAINING_BUILD_ARG_DEFAULTS["use_sh_stage2"]; use_sh_stage3: bool = TRAINING_BUILD_ARG_DEFAULTS["use_sh_stage3"]; use_sh_stage4: bool = TRAINING_BUILD_ARG_DEFAULTS["use_sh_stage4"]
    sh_band_stage1: int = TRAINING_BUILD_ARG_DEFAULTS["sh_band_stage1"]; sh_band_stage2: int = TRAINING_BUILD_ARG_DEFAULTS["sh_band_stage2"]; sh_band_stage3: int = TRAINING_BUILD_ARG_DEFAULTS["sh_band_stage3"]; sh_band_stage4: int = TRAINING_BUILD_ARG_DEFAULTS["sh_band_stage4"]
    ...
    def __post_init__(self) -> None:
        ...
        self.raster_grad_distance_power = float(self.raster_grad_distance_power)
        self.raster_grad_distance_bias = float(self.raster_grad_distance_bias)
        ...
        self.refinement_ema_pose_count_decay = float(np.clip(self.refinement_ema_pose_count_decay, 0.0, 1.0))
        self.refinement_viewed_fraction_zero_threshold = max(float(self.refinement_viewed_fraction_zero_threshold), 0.0)
...
class GaussianTrainer:
    ...
    _KERNEL_ENTRIES = {
        ...
        "clear_current_contribution": (Path(SHADER_ROOT / "renderer" / "gaussian_training_stage.slang"), "csClearCurrentContribution"),
        "update_visible_average_contribution": (Path(SHADER_ROOT / "renderer" / "gaussian_training_stage.slang"), "csUpdateVisibleAverageContribution"),
        ...
        "prepare_refinement_prune_sort_inputs": (Path(SHADER_ROOT / "renderer" / "gaussian_training_stage.slang"), "csPrepareRefinementPruneSortInputs"),
        "mark_refinement_prune_mask": (Path(SHADER_ROOT / "renderer" / "gaussian_training_stage.slang"), "csMarkRefinementPruneMask"),
    ...
    def refinement_clone_budget(self, splat_count: int | None = None, step: int | None = None) -> int:
    ...
    def _dispatch_raster_backward(
        ...
        self.renderer.rasterize_backward_current_scene(
            ...
            grad_scale=1.0,
            raster_grad_distance_power=float(self.training.raster_grad_distance_power),
            raster_grad_distance_bias=float(self.training.raster_grad_distance_bias),
    ...
    def _refinement_sampling_seed(self) -> int:
        return int(_refinement_hash_combine(self._seed + 0x51A7D3C5, self.state.step + 1))

    def _update_refinement_prune_mask(self) -> None:
        ...
        self._prefix_sum.scan_uint(
            ...
            self._refinement_buffers["refinement_prune_prefix"],
            self._scene_count,
            self._refinement_buffers["total_clone_counter"],
            exclusive=False,
        )
        self._sorter.sort_key_values(
            encoder=enc,
            keys_buffer=self._refinement_buffers["refinement_prune_sort_keys"],
            values_buffer=self._refinement_buffers["refinement_prune_sort_values"],
            n=self._scene_count,
            copy_result_back=True,
        )
        self._dispatch(
            "mark_refinement_prune_mask",
            enc,
            thread_count_1d(self._scene_count),
            self._refinement_vars(),
        )
    ...
    def _dispatch_refinement_clone_sampling(self) -> None:
        if self._scene_count <= 0:
            return
        self._update_refinement_prune_mask()
        clone_budget = max(self.refinement_clone_budget(step=self.state.step), 0)
        enc = self.device.create_command_encoder()
        self._dispatch(
            "prepare_refinement_sampling_weights",
            enc,
            spy.uint3(max(self._scene_count, 1), 1, 1),
            self._refinement_vars(refinement_sample_count=clone_budget),
        )
        self._prefix_sum.scan_float(
            enc,
            self._refinement_buffers["refinement_weights"],
            self._refinement_buffers["refinement_weight_prefix"],
            self._scene_count,
            self._refinement_buffers["refinement_weight_total"],
            exclusive=False,
        )
        if clone_budget > 0:
            self._dispatch(
                "sample_refinement_clone_counts",
                enc,
    ...
    def _sample_refinement_clone_counts(self) -> tuple[np.ndarray, np.ndarray]:
        self._dispatch_refinement_clone_sampling()
        clone_counts = buffer_to_numpy(self._refinement_buffers["clone_counts"], np.uint32)[: self._scene_count].copy()
    ...
    def _refinement_vars(
        ...
        *,
        ...
        return {
            ...
            "g_RefinementEligibleMask": self._refinement_buffers["refinement_eligible_mask"],
            "g_RefinementPruneMask": self._refinement_buffers["refinement_prune_mask"],
            "g_RefinementPruneSortKeys": self._refinement_buffers["refinement_prune_sort_keys"],
            "g_RefinementPruneSortValues": self._refinement_buffers["refinement_prune_sort_values"],
            "g_RefinementPrunePrefix": self._refinement_buffers["refinement_prune_prefix"],
            ...
            "g_RefinementSamplingSeed": np.uint32(self._refinement_sampling_seed()),
            ...
            "g_RefinementMinContributionThreshold": float(refinement_threshold),
            ...
            "g_RefinementSolveOpacity": np.uint32(1 if self.training.refinement_solve_opacity else 0),
            "g_RefinementSplitBeta": float(self.training.refinement_split_beta),
            ...
            "g_RefinementViewedFractionZeroThreshold": float(self.training.refinement_viewed_fraction_zero_threshold),
    ...
    def sorting_dither(self, frame_index: int, step: int, camera: Camera) -> SortCameraDither:
        amount = resolve_sorting_order_dithering(self.training, int(step))
    ...
    def _ensure_ssim_buffers(self) -> None:
        ...
        self._buffers["ssim_feature_grads"] = self._ssim_blur.make_buffer(self._SSIM_FEATURE_CHANNELS, name="trainer.ssim_feature_grads")
    ...
    def _ensure_post_refinement_source_buffer_capacity(self, splat_count: int) -> None:
        ...
        self._set_refinement_buffer("splat_contribution", contribution)
        ...
        self._set_refinement_buffer("refinement_prune_sort_values", alloc_buffer(self.device, name="trainer.refinement.prune_sort_values", size=self._refinement_splat_capacity * self._U32_BYTES, usage=RW_BUFFER_USAGE))
        self._set_refinement_buffer("refinement_prune_prefix", alloc_buffer(self.device, name="trainer.refinement.prune_prefix", size=self._refinement_splat_capacity * self._U32_BYTES, usage=RW_BUFFER_USAGE))
        ...
        if "gradient_stats" in self._refinement_buffers:
            gradient_stats = alloc_buffer(self.device, name="trainer.refinement.gradient_stats", size=self._refinement_splat_capacity * self._GRAD_STATS_STRIDE * self._U32_BYTES, usage=RW_BUFFER_USAGE)
            gradient_stats.copy_from_numpy(np.zeros((self._refinement_splat_capacity, self._GRAD_STATS_STRIDE), dtype=np.float32))
            self._set_refinement_buffer("gradient_stats", gradient_stats)
    ...
    def _ensure_refinement_buffers(self, splat_count: int, append_count: int | None = None) -> None:
        ...
        if "refinement_prune_mask" not in self._refinement_buffers:
            self._refinement_buffers["refinement_prune_mask"] = alloc_buffer(self.device, name="trainer.refinement.prune_mask", size=self._refinement_splat_capacity * self._U32_BYTES, usage=RW_BUFFER_USAGE)
        if "refinement_prune_sort_keys" not in self._refinement_buffers:
            self._refinement_buffers["refinement_prune_sort_keys"] = alloc_buffer(self.device, name="trainer.refinement.prune_sort_keys", size=self._refinement_splat_capacity * self._U32_BYTES, usage=RW_BUFFER_USAGE)
        if "refinement_prune_sort_values" not in self._refinement_buffers:
            self._refinement_buffers["refinement_prune_sort_values"] = alloc_buffer(self.device, name="trainer.refinement.prune_sort_values", size=self._refinement_splat_capacity * self._U32_BYTES, usage=RW_BUFFER_USAGE)
        if "refinement_prune_prefix" not in self._refinement_buffers:
            self._refinement_buffers["refinement_prune_prefix"] = alloc_buffer(self.device, name="trainer.refinement.prune_prefix", size=self._refinement_splat_capacity * self._U32_BYTES, usage=RW_BUFFER_USAGE)
        ...
        if "refinement_weight_prefix" not in self._refinement_buffers:
    ...
    def _refinement_camera_signature_value(self) -> tuple[object, ...]:
        signature: list[object] = [
            ...
            round(float(self.training.camera_min_dist), 8),
            int(len(self.frames)),
        ]
    ...
    def _clear_clone_counts(self, *, preserve_refinement_history: bool = False, keep_contribution_history: bool = False) -> None:
        zero_counter = np.zeros((1,), dtype=np.uint32)
        zero_clone_counts = np.zeros((max(self._refinement_splat_capacity, 1),), dtype=np.uint32)
        self._refinement_buffers["total_clone_counter"].copy_from_numpy(zero_counter)
        self._refinement_buffers["append_counter"].copy_from_numpy(zero_counter)
        self._refinement_buffers["clone_counts"].copy_from_numpy(zero_clone_counts)
        if "refinement_prune_mask" in self._refinement_buffers:
    ...
    def _run_refinement(self, clone_counts_override: np.ndarray | None = None) -> None:
        ...
        self._ensure_post_refinement_source_buffer_capacity(self._scene_count)
        self._ensure_training_buffers(self._scene_count, 1)
        ...
        self._clear_clone_counts(keep_contribution_history=True)
    ...
    def _ensure_compensated_native_target_texture(self, frame_index: int) -> spy.Texture:
        ...
        texture = alloc_texture_2d(
            self.device,
            name="trainer.frame_target_compensated",
            format=spy.Format.rgba32_float,
            width=width,
            height=height,
            usage=spy.TextureUsage.shader_resource | spy.TextureUsage.unordered_access,
    ...
    def _refresh_compensated_native_target(self, encoder: spy.CommandEncoder, frame_index: int) -> spy.Texture:
        ...
        self._dispatch(
            ...
            thread_count_2d(int(frame.width), int(frame.height)),
            {
                ...
                "g_PhotometricTargetWidth": int(frame.width),
                "g_PhotometricTargetHeight": int(frame.height),
    ...
    def _dispatch_downscale_target(self, encoder: spy.CommandEncoder, frame_index: int, step: int | None = None) -> None:
        ...
        self._dispatch(
            ...
            self._pixel_thread_count(),
            {
                "g_SourceTarget": self._native_target_texture(frame_index, encoder),
    ...
    def step_batch(self, step_count: int) -> int:
        ...
        with debug_region(enc, "Trainer Batch", 48):
            for batch_index in range(batch_steps):
                with debug_region(enc, "Trainer Step", 49):
    ...
    def evaluate_frame_metrics(
        ...
        *,
        ...
        if update_runtime_metrics:
            ...
            self.state.last_mse = metrics.mse
            self.state.last_ssim = metrics.ssim
            self.state.last_psnr = metrics.psnr
            self.state.last_frame_index = metrics.frame_index
            self.state.avg_loss = self._frame_metrics.mean("loss")
            self.state.avg_mse = self._frame_metrics.mean("mse")
            self.state.avg_ssim = self._frame_metrics.mean("ssim")
            self.state.avg_psnr = self._frame_metrics.mean("psnr")
