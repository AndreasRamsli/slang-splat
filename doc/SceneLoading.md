...
- `sample_colmap_fibonacci_sphere_points(...)` builds optional shell points around the arithmetic mean of the aligned COLMAP camera centers. The viewer still appends those points to point-cloud initializers, but the appended shell now gets its own equal-area overlap scale derived only from `(shell radius, shell point count)` instead of inheriting the sparse-cloud nearest-neighbor scale coefficient.
