...
- The same reverse walk also maintains a suffix color state for training contribution tracking. For each processed splat it atomically accumulates `T_prefix * alpha * length(splat_rgb - rgb_behind)` into `g_SplatContribution`, where `rgb_behind` includes splats behind the current one plus the resolved training background; alpha-only output changes are intentionally ignored. When target alpha masking is enabled, masked target pixels skip this contribution add.
