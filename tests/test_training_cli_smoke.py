...
def test_train_cli_parser_defaults_color_and_opacity_lr_mul_to_five() -> None:
    ...
    assert kwargs["position_push_away_from_camera_step"] == float(TRAINING_BUILD_ARG_DEFAULTS["position_push_away_from_camera_step"])
...
def test_train_cli_refinement_distribution_exponent_flags_and_alias() -> None:
    ...
    args = parser.parse_args(["train-colmap", "--colmap-root", "dummy", "--refinement-grad-variance-weight-exponent", "2.25", "--refinement-contribution-weight-exponent", "3.25", "--refinement-contribution-area-exponent", "0.75", "--refinement-contribution-view-count-exponent", "1.25"])
