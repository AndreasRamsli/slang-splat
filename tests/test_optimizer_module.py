...
def test_generic_packed_regularize_supports_per_parameter_target_pull_and_abs_shrink(device):
    regularize_shader_path = Path("shaders/utility/optimizer/optimizer.slang")
    regularize_kernel = device.create_compute_kernel(device.load_program(str(regularize_shader_path), ["csRegularizePacked"]))
    param_count = 4
    usage = spy.BufferUsage.shader_resource | spy.BufferUsage.unordered_access | spy.BufferUsage.copy_source | spy.BufferUsage.copy_destination
    params_init = np.array([1.0, -2.0, 3.0, -4.0], dtype=np.float32)
    settings = np.zeros((param_count, _SETTINGS_U32_WIDTH), dtype=np.uint32)
    ...
    params_after = np.frombuffer(params.to_numpy().tobytes(), dtype=np.float32)[:param_count].copy()
    expected = np.array([1.5, -1.0, 2.8, -2.5], dtype=np.float32)
    np.testing.assert_allclose(params_after, expected, rtol=0.0, atol=1e-6)
