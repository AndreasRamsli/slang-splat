...
from tests.viewer_test_harness import (
    ...
    _DebugViewportRenderer,
    _FinishedEncoder,
    _TRAINING_VISUAL_CACHE_DEFAULTS,
    _TonemapAwareTrainer,
    _assign_training_renderer,
    _build_colmap_tree,
    _build_colmap_tree_without_database,
    _capturing_gaussian_trainer,
    _identity_q,
    _make_import_viewer,
    _make_training_scene_viewer,
    _patch_import_pipeline,
    _patch_import_runtime_resets,
...
def test_reinitialize_training_scene_reuses_existing_native_targets(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda viewer_obj, frame_targets_native=None, preserve_session_state=False: captured.append((frame_targets_native, preserve_session_state)),
    )

    session.reinitialize_training_scene(viewer)

    assert captured == [(textures, True)]
...
def test_reinitialize_training_scene_preserves_non_gaussian_state(monkeypatch) -> None:
    ...
    assert viewer.s.photometric_resume_time == 11.0
    assert viewer.s.last_error == "stale"
    assert viewer.s.training_runtime_factor_changed is False
    assert viewer.s.pending_training_runtime_resize is False
    assert viewer.s.last_training_batch_steps == 0
    assert new_trainer.target_tonemap_provider is provider
...
def test_initialize_training_scene_does_not_preserve_old_native_targets_on_fresh_import(monkeypatch) -> None:
    ...
    assert calls[0] == ("release", False)
    assert captured["frame_targets_native"] == new_targets
...
def test_ensure_training_runtime_resolution_reuses_larger_capacity_without_reset(monkeypatch) -> None:
    trainer = SimpleNamespace(
    ...
    training_renderer = SimpleNamespace(width=64, height=64, _render_capacity_width=64, _render_capacity_height=64, work_buffers={"debug_grad_norm": "grad_norm"})
    viewer = SimpleNamespace(
        s=SimpleNamespace(
            trainer=trainer,
    ...
    assert viewer.s.training_renderer is training_renderer
    assert viewer.s.applied_training_runtime_factor == 2
